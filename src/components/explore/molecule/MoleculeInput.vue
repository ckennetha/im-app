...
const props = defineProps({
  useMoleculeDrawer: { type: Boolean, required: true },
  count: { type: Number, default: 0 },
  maxMolecules: { type: Number, default: 1 },
})
...
watch(isMoleculeLimit, (newValue) => {
  newValue ? emit("update:limitReached", true) : emit("update:limitReached", false)
}, { immediate: true })
</script>
  ...
  <div>
    ...
    <div class="flex flex-row w-full justify-between pb-1">
      <div v-if="!useMoleculeDrawer" class="flex flex-col flex-2 max-w-full">
        <Textarea id="input-molecule"
          v-model="model"
          :class="cn('textarea text-sm focus-visible:ring-0', maxMolecules > 1 && 'h-55')"
      ...
      <div v-else class='flex flex-3'>
        <iframe id="ketcher" src="/ketcher/index.html" class="w-full h-100 border rounded-md shadow-xs" />
      </div>
    </div>
